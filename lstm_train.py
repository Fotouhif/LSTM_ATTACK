...
def train_iterators(source_folder, batch_size, device):
    ...
    valid_iter = BucketIterator(valid, batch_size=batch_size, sort_key=lambda x: len(x.review), device=device, sort=True, sort_within_batch=True)
    test_iter = BucketIterator(test, batch_size=batch_size, sort_key=lambda x: len(x.review), device=device, sort=True, sort_within_batch=True)
...
class LSTM(nn.Module):
    ...
    def __init__(self, text_field, dimension=128):
        ...
        self.embedding = nn.Embedding(len(self.text_field.vocab), 300) #an Embedding module containing 676135 tensors of size 300
...
class train_model:
    def __init__(self, source_folder, batch_size, device, num_epochs, learning_rate):
    ...
    def model_training(self):
        train_iter, valid_iter, test_iter, text_field = train_iterators(self.source_folder, self.batch_size, self.device)
        model = LSTM(text_field=text_field).to(self.device)
        optimizer = optim.Adam(model.parameters(), lr=self.learning_rate)

        train(model=model, optimizer=optimizer, num_epochs=self.num_epochs, train_loader=train_iter, valid_loader=valid_iter, device=self.device)
